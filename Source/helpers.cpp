...
*/
void write_compressed_page_z80 (FD& fd, int page_id , uint8 const* q, uint32 qsize) throw(file_error)
{
...
/*	write block of data in intel hex file format
	...
	where
		...
		\r\n = dos line end
*/
void write_intel_hex( FD& fd, uint32 addr, uint8 const* bu, uint32 sz ) throw(file_error)
{
	if(sz==0) return;
...
/*	write compressed data in .ace format
		...
		compression scheme:
			dc.b $ed, count, char
*/
void write_compressed_page_ace (FD& fd, uint8 const* q, uint qsize) throw(file_error)
{
